...
int main(int argc, char *argv[]) {
    ...
    if(argc!=3){//error checks proper number of command line arguments
    ...
    if(fp==NULL || fp2==NULL) {//error checks to see if files can be used
    ...
    if(index==-1) //error checks to see if valid player was entered
    ...
    freeMem(goals,assists,names,find,size);//frees all pointers
